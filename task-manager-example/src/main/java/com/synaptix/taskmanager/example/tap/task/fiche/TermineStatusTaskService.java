...
public class TermineStatusTaskService extends AbstractTaskService {
    ...
    public IExecutionResult execute(IEngineContext context, ICommonTask commonTask) {
        ...
        if (operations != null && !operations.isEmpty()) {
            long nb = operations.stream().filter(operation -> FicheContactStatus.TERMINE.equals(operation.getEndFicheContactStatus()) && !OperationStatus.DONE.equals(operation.getOperationStatus()))
                    .count();
            if (nb > 0) {

...
public class EtudeStatusTaskService extends AbstractTaskService {
    ...
    public IExecutionResult execute(IEngineContext context, ICommonTask commonTask) {
        ...
        if (operations != null && !operations.isEmpty()) {
            long nb = operations.stream().filter(operation -> FicheContactStatus.ETUDE.equals(operation.getEndFicheContactStatus()) && !OperationStatus.DONE.equals(operation.getOperationStatus()))
                    .count();
            if (nb > 0) {
